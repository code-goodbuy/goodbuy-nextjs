...
export default function InfoCard({ number, unit, text, url }: InfoCardType) {
	...
	function getPartialNumber(number: number) {
		/**
	...
	const partialNumber = parseInt(getPartialNumber(number).toFixed(0)).toLocaleString();
	return (
		<div className="rounded-2xl ring-2 ring-primary dark:ring-secondary p-5 w-75 md:w-50 lg:w-30 mt-10">
			...
			<a className="colorful-button cursor-pointer" href={url}>
				Learn More
			</a>
		</div>
	);
